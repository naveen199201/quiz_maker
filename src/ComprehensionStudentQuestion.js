    ...
    <div className="comprehension-quiz-question">
      {question.questions.map((mcq, qindex) => (
        <div key={qindex} className="mcq">
          ...
          <p className="mcq-question">{mcq.text}</p>
          <button className="reset-button" onClick={() => handleReset(qindex)}>
            Reset
          </button>
            ...
            {mcq.options.map((option, optionIndex) => (
              <div key={optionIndex} className="option">
                 <input
                  ...
                  />
                <label htmlFor={`q${qindex}_t${question._id}_o${optionIndex}`}>{option}</label>
              </div>
            ))}
          </div>
